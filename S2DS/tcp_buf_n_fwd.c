...
int main(int argc, char *argv[])
{
	...
	do
	{
		if (wait_for_clients() == 0)
		{
			...
			printf("invalid client connection occured!\n");
		}
	}
...
int use_tunnel(void)
{
	...
	for (;;)
	{
		...
		if (FD_ISSET(rc.client_socket, &io))
		{
			...
			if (settings.log)
			{
				printf("@ %s: forwarded %d bytes from %s:%d to remote(%s:%d)\n", \
					   get_current_timestamp(), \
					   count,\
					   inet_ntoa(rc.client_addr.sin_addr), \
					   rc.client_addr.sin_port,\
					   inet_ntoa(rc.remote_addr.sin_addr), \
					   rc.remote_addr.sin_port);
		...
		if (FD_ISSET(rc.remote_socket, &io))
		{
			...
			if (settings.log)
			{
				printf("@ %s: forwarded %d bytes from %s:%d to client(%s:%d)\n", \
					   get_current_timestamp(), \
					   count,\
					   inet_ntoa(rc.remote_addr.sin_addr), \
					   rc.remote_addr.sin_port,\
